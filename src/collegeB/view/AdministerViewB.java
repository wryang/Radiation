...
import collegeB.controller.AdministerViewControllerB;

public class AdministerViewB {
	private AdministerViewControllerB controller;
	...
	private ArrayList<StudentB> stdList;
	private ArrayList<CourseB> courseList;
	private StudentB stdSelected;
	private CourseB courseSelected;
	...
	private void initialize() {
		...
		// student information
		stdList = new ArrayList<StudentB>();
		stdSelected = new StudentB();
		...
		// course information
		courseList = new ArrayList<CourseB>();
		courseSelected = new CourseB();
		...
		delStdInfoBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if (stdTable.getSelectedRow() >= 0) {
					...
					int choice1 = JOptionPane.showConfirmDialog(null,
							"确定要删除该学生资料吗？", null, JOptionPane.OK_CANCEL_OPTION);
					if (choice1 == 0) {
						stdSelected = stdList.get(stdTable.getSelectedRow());
		...
		detailCourseInfoBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if (courseTable.getSelectedRow() < 0) {
					...
					courseSelected = courseList.get(courseTable
							.getSelectedRow());
		...
		delCourseInfoBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if (courseTable.getSelectedRow() >= 0) {
					JLabel temp = new JLabel();
					int choice1 = JOptionPane.showConfirmDialog(null,
							"确定要删除该课程吗？", null, JOptionPane.OK_CANCEL_OPTION);
					if (choice1 == 0) {
						courseSelected = courseList.get(courseTable
								.getSelectedRow());
	...
	private void drawStdTable() {
		...
		final String[] ColumnName = { "学号", "姓名", "性别", "专业" };
		final int row;
		...
		stdTableModel = new AbstractTableModel() {
			...
			public Object getValueAt(int row, int column) {
				if (stdList != null && stdList.size() != 0) {
					...
					switch (column) {
					case 0:
						return stdSelected.getStudentNumber(); // get student
																// number
					case 1:
						return stdSelected.getName(); // get name
					case 2:
					...
					case 3:
						return stdSelected.getSpeciality(); // get college
					}
				}
				return null;
			}
		...
		for (int column = 0; column < 4; column++)
			stdTable.getTableHeader().getColumnModel().getColumn(column)
					.setHeaderValue(ColumnName[column]);
	...
	private void drawCourseTable() {
		final String[] ColumnName = { "课程编号", "课程名称", "授课老师", "课时", "学分",
				"授课地点" };
		...
		courseTableModel = new AbstractTableModel() {
			...
			public Object getValueAt(int row, int column) {
				if (courseList != null && courseList.size() != 0) {
					...
					switch (column) {
					case 0:
						return courseSelected.getId(); // get student number
					case 1:
						return courseSelected.getCourseName(); // get name
					case 2:
						return courseSelected.getTeacher(); // get sex
					case 3:
						return courseSelected.getPeriod(); // get college
					case 4:
						return courseSelected.getPoint(); // get college
					case 5:
						return courseSelected.getAddress();
					}
		...
		for (int column = 0; column < 6; column++)
			courseTable.getTableHeader().getColumnModel().getColumn(column)
					.setHeaderValue(ColumnName[column]);
	...
	public void setController(AdministerViewControllerB controller) {
	...
	public StudentB getStdSelected() {
		// TODO Auto-generated method stub
		return stdSelected;
	}

	public CourseB getCourseSelected() {
		// TODO Auto-generated method stub
