...
import collegeB.controller.CourseSelectedStuControllerB;

public class CourseSelectedStuViewB {
	CourseSelectedStuControllerB controller;
	JFrame infoFrame ;
	JPanel infoPanel;
	JTable studentTable;
	JButton addBtn;
	...
	public CourseSelectedStuViewB(CourseB cunrrentCourse){
		init();
		addActionListener();
		controller=new CourseSelectedStuControllerB(this,cunrrentCourse);
	}
...
//				infoFrame.dispose();
	...
	public void updateStudentTable(List<StudentB> allStudents){
		int a =myModel.getRowCount();
		for(int i=a;i>1;i--)
			myModel.removeRow(1);
		for(int i=0;i<allStudents.size();i++){
			myModel.insertRow(i+1, new String[]{allStudents.get(i).getStudentNumber(),allStudents.get(i).getName()});
		}
