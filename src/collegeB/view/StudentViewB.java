...
import collegeB.view.MyTableModel;
import collegeB.bean.CourseB;
import collegeB.controller.StudentViewControllerB;

public class StudentViewB {
	StudentViewControllerB controller;
	...
	public StudentViewB(){
		init();
		addActionListener();
		controller=new StudentViewControllerB(this);
	}
...
//		// TODO Auto-generated method stub]
//		controller.chooseBtnClicked((String)courseTable.getValueAt(selectIndex, 0));
////		updateCourseInfo();
//	}
...
public void updateCourseTable(List<CourseB> allcourse){
	for(int i=0;i<allcourse.size();i++){
		myModel.insertRow(1, new String[]{allcourse.get(i).getId(),allcourse.get(i).getCourseName(),allcourse.get(i).getTeacher(),allcourse.get(i).getPoint(),allcourse.get(i).getAddress(),allcourse.get(i).getPeriod()});
	}
...
public void updateCourseInfo(String courseName){
...
public void updateSelectTable(List<CourseB> courses) {
//	if(null == courses) {
//		return;
//	}
	// TODO Auto-generated method stub
...
//行的序号随着删除的进行而改变了！！
	...
	for(int i=0;i<courses.size();i++){
		selectModel.insertRow(i+1, new String[]{courses.get(i).getId(),courses.get(i).getCourseName()});
